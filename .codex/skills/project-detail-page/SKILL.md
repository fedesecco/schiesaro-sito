...
- Subito sotto il titolo, prima dell'immagine, mostrare una sola riga di coordinate composta da latitudine e longitudine divise in major/minor; non duplicare i valori in righe o blocchi aggiuntivi.
- Area contenuti: carousel orizzontale con snap al centro; la slide testuale occupa una colonna editoriale leggibile, le immagini usano dimensioni proporzionate all'orientamento.
- Il blocco metadata parte vicino al chrome, senza un margine superiore sproporzionato.
- La slide testuale usa un corpo serif contenuto; se il testo eccede l'altezza desktop, il viewport consente lo scroll verticale nativo.
- La slide immagine mostra soltanto la media, senza caption, credit, indice locale o istruzioni attaccate all'immagine; usare `height: auto` e `object-fit: contain` senza crop arbitrario.
- Le immagini occupano quasi tutta l'area disponibile sotto chrome e coordinate, lasciando solo il margine necessario all'indice globale.
...
- Click sui bordi desktop: precedente/successivo; il bordo destro deve essere un controllo cliccabile esplicito, non solo un'area decorativa. Click al centro: snap alla slide più vicina.
- Il cursore di base non è una mano permanente: usare cursore neutro nel viewport e cursori direzionali sui controlli bordo.
- Offrire anche controlli espliciti precedente/successivo o un equivalente accessibile, così la navigazione non dipende da mouse, swipe o coordinate dello schermo.
...
2. Desktop rispetta la lettura orizzontale dei mockup; mobile rispetta la lettura verticale dei mockup.
3. Le immagini non mostrano didascalie, crediti o hint; il footer desktop mostra soltanto `n/totale` e quello mobile aggiunge solo eventuali controlli iconici necessari.
4. Le coordinate del progetto appaiono una sola volta tra titolo e viewport immagini.
5. I controlli da tastiera/screen reader permettono di raggiungere ogni slide e tornare a `/home`.
6. `slide` resta coerente dopo click, drag, rotella, refresh e link diretto.
7. Missing project e asset non disponibile hanno una resa esplicita, senza placeholder grafici.
